...
phoneme_labels = joblib.load('../TRAIN/phoneme_labels.joblib')  
...
test_data = np.zeros((26,1)); test_class = []; 

for p in phonemes: 
    np.random.shuffle(phonemes[p].T);
    test_data = np.concatenate((test_data, phonemes[p][:,0:min_num]), axis=1)
    test_class = np.append(test_class, np.repeat(phoneme_labels.index(p), min_num))

test_data = np.delete(test_data, 0, axis=1);
testing_data = {'test_data':test_data, 'test_class': test_class, 'phonemes':phoneme_labels}
joblib.dump(testing_data, 'testing_data.joblib')
